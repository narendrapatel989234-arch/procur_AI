...
const INITIAL_NODES = [
  ...
  { id: 1, type: 'ai', status: 'waiting', title: 'Auto classification', actor: 'AI Agent', timestamp: null, icon: 'GitBranch' },
  { id: 2, type: 'user', status: 'waiting', title: 'Submit PR', actor: 'User', timestamp: null, icon: 'User' },
...
export default function PRDetailFresh({ onNavigate, userRole, navState }) {
  ...
  useEffect(() => {
    const T = [
      ...
      setTimeout(() => { upd([2, 3], 'active'); setPrStatus('Budget Check'); }, 4500),
      setTimeout(() => upd([2, 3], 'complete'), 7500),
      setTimeout(() => { upd([4, 5], 'active'); setPrStatus('Submitted'); }, 8000),
      setTimeout(() => upd([4, 5], 'complete'), 10500),
      setTimeout(() => { upd([7, 8], 'active'); setPrStatus('Routing & Sourcing'); }, 11000),
      setTimeout(() => upd([7, 8], 'complete'), 15000),
      setTimeout(() => { upd([9], 'active'); setPrStatus('PO Generated'); }, 15500),
      setTimeout(() => upd([9], 'complete'), 18000),
      setTimeout(() => { upd([10], 'pending_user'); setPrStatus('Approval Pending'); }, 18500),
    ];
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#fff' }}>
        ...
        <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
          ...
          {activeTab === 'activities' && (
            <div style={{ flex: 1, overflowY: 'auto', background: '#f5f5f7', padding: '32px 40px' }}>
                      ...
                      <div key={entry.id} style={{ display: 'flex', gap: 16, position: 'relative' }}>
                        ...
                        <div style={{ flex: 1, background: '#fff', border: '1px solid #e5e5e5', borderRadius: 12, padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.02)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                            <div style={{ fontSize: 14, fontWeight: 600, color: '#1a1a1a' }}>{entry.title}</div>
                            <div style={{ fontSize: 12, color: '#999' }}>{entry.time}</div>
                          </div>
